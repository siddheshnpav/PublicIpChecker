...
public class CheckOldandNewIP {
	...
	public static void checkandUpdateNewIP() {
		...
		if (publicIP == null) {
			...
			Utils.UpdateIPCheckerStatusINI("LastRunStatus", "No Internet Connection is Available");
			return;
		}
		...
		if (oldIp != null && oldIp.equals(publicIP)) {
			...
			if (isSuccessTelegramPostStatus == null || isSuccessTelegramPostStatus.equals("FALSE")) {
				try {
					Utils.writeToLog(
							"Previous Telegram Alert was not successful. Attempting to resend alert for current IP: "
									+ publicIP);
					Utils.UpdateIPCheckerStatusINI("LastRunStatus",
							"No IP Change Detected but Previous Telegram Alert was not successful, Attempting to Resend Alert");
					TelegramAPIBot.sendTelegramAlert(publicIP);

				} catch (IOException e) {
					Utils.writeToLog("Error while sending Telegram Alert");
					Utils.UpdateIPCheckerStatusINI("LastRunStatus",
							"IP Change Detected but Failed to Send Telegram Alert");
			...
			if (isSuccessTelegramPostStatus != null && isSuccessTelegramPostStatus.equals("TRUE")) {
				Utils.writeToLog(
						"Previous Telegram Alert was successful. No need to resend alert for current IP: " + publicIP);
				Utils.UpdateIPCheckerStatusINI("LastRunStatus",
						"No IP Change Detected and Previous Telegram Alert was successful, No need to Resend Alert");
			}
